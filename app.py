...
st.set_page_config(
     page_title="Repetition Counting online",
     page_icon=":shark:",
     layout="wide",
...
st.write('# Repetition Counting Online made by Nguyen Viet Duc')
import base64
import io
import os
import time
...
PATH_TO_CKPT = '/content/drive/MyDrive/repnet/tmp/repnet_ckpt'
# !mkdir $PATH_TO_CKPT
...
with st.sidebar:
    WITHIN_PERIOD_THRESHOLD = st.slider('WITHIN_PERIOD_THRESHOLD', 0.0, 1.0, 0.5)
...
with st.sidebar:
    # VIZ_FPS = st.slider('VIZ_FPS', 15, 30, 30)
    VIZ_FPS = st.number_input('VIZ_FPS', min_value=15, max_value=60, value=30, step=1)
...
st.write('## Please enter video link: ')
# Hummingbird flying.
# VIDEO_URL = 'https://imgur.com/t/hummingbird/m2e2Nfa'


with st.form("my_form"):
  VIDEO_URL = st.text_input(' ', 'https://www.youtube.com/watch?v=5g1T-ff07kM')
  submitted = st.form_submit_button("Submit", on_click=None)
  # time.sleep(20)
  if submitted:
    st.write(' ')

# VIDEO_URL = st.text_input(' ', 'https://imgur.com/t/hummingbird/m2e2Nfa')
# time.sleep(20)
...
with st.spinner('Processing...'):
  ...
  video_file = open('/tmp/debug_video.mp4', 'rb')
